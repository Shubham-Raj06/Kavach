...
const FeaturePipelinePanel = dynamic(() => import('../components/FeaturePipelinePanel'), { ssr: false });
const AnomalyInsightPanel = dynamic(() => import('../components/AnomalyInsightPanel'), { ssr: false });
const ForecastPanel = dynamic(() => import('../components/ForecastPanel'), { ssr: false });
const TransparencyModal = dynamic(() => import('../components/TransparencyModal'), { ssr: false });
const MLStatusPanel = dynamic(() => import('../components/command/MLStatusPanel'), { ssr: false });
const ModelObservabilityPanel = dynamic(() => import('../components/command/ModelObservabilityPanel'), { ssr: false });
...
export default function Dashboard() {
    ...
    const handleWardClick = useCallback((ward) => {
        ...
        // In a real app, you'd fetch the specific ML response for this ward here
        // For now we just pass the ward object as a proxy
        if (ward) setMlResponse({ wardId: ward.wardId, riskScore: ward.riskScore, features: ward.features });
    }, [setSelectedWard]);
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, minHeight: 0 }}>
                        ...
                        <div style={{
                            ...
                            <div style={{ flex: 1, padding: '12px 14px', overflowY: 'auto' }}>
                                ...
                                {activeTab === 'ml' && selectedWard && (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 12 }}>
                                            <AnomalyInsightPanel
                                                anomalyData={{ isAnomaly: selectedWard.isAnomaly, anomalyScore: selectedWard.isAnomaly ? -0.15 : 0.05, anomalyFeatures: selectedWard.isAnomaly ? ['AQI', 'Temperature'] : [] }}
                                                scaledFeatures={{ Temperature: 1.2, Humidity: -0.5, AQI: 2.4, WQI: 0.1, Rainfall: -0.2 }}
                                            />
                                            <FeaturePipelinePanel
                                                rawFeatures={{ temp_avg: 34, humidity_avg: 65, aqi: 240, wqi: 6.5 }}
                                                scaledFeatures={{ temp_avg: 1.2, humidity_avg: -0.5, aqi: 2.4, wqi: 0.1 }}
                                            />
                                        </div>
                                        <button
                                            ...
                                            style={{
                                                ...
                                                color: '#93c5fd', padding: '10px', borderRadius: 8, cursor: 'pointer',
                                                fontFamily: 'monospace', fontSize: 11, fontWeight: 600,
                                            }}
                                        >
                                            🔎 OPEN FULL TRANSPARENCY DRILLDOWN
                                        </button>
                                    </div>
                                )}
                                {activeTab === 'forecast' && selectedWard && (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                        <ForecastChart ward={selectedWard} />
                                        <ForecastPanel wardId={selectedWard.wardId} />
                                    </div>
                                )}
